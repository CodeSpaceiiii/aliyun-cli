...
var OptionMap = map[string]Option{
	...
	OptionSTSRegion: Option{"", "--sts-region", "", OptionTypeString, "", "",
		"指定sts endpoint的地区，比如cn-shenzhen，其中，cn指代的是国家，shenzhen指代的是地区，用于构造sts endpoint，该选项缺省时，sts endpoint为" + setting.STS_DEFAULT_DOMAIN + "，主要用于RamRoleArn模式",
		"specifies the region of sts endpoint, such as cn-shenzhen, in this case, cn refers to the country and shenzhen refers to the region, to construct sts endpoint, when this option defaults, the sts endpoint is " + setting.STS_DEFAULT_DOMAIN + ", primarily used in RamRoleArn mode."},
