...
func (s *OssutilConfigSuite) TestConfigNonInteractive(c *C) {
	...
	endpoint := setting.OSS_DEFAULT_REGION + "." + setting.DOMAIN_SUFFIX
	accessKeyID := "ak"
	accessKeySecret := "sk"
	stsToken := "token"
...
func (s *OssutilConfigSuite) TestConfigNonInteractiveWithCommonOption(c *C) {
	...
	readTimeout := "10"
	connectTimeOut := "10"
	stsRegion = "sts.cn-qingdao." + setting.DOMAIN_SUFFIX
	signVersion := "v4"
	region := "oss-cn-chengdu." + setting.DOMAIN_SUFFIX
	cloudboxId := "12124123"
	retryTimes := "400"
	data := "[Credentials]" + "\n" +
	...
	stsRegion1 := "sts.cn-hangzhou." + setting.DOMAIN_SUFFIX
	data = "[Credentials]" + "\n" +
		"language=" + DefaultLanguage + "\n" +
		"accessKeyID=" + accessKeyID + "\n" +
		"accessKeySecret=" + accessKeySecret + "\n" +
